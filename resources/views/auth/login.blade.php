...
<head>
    ...
    <meta name="description" content="{{ config('app.name', 'Fursaana') }} - تسجيل الدخول إلى منصة التوظيف الرائدة">

    <title>{{ config('app.name', 'Fursaana') }} - تسجيل الدخول</title>
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-4 py-2 rounded-full text-sm font-medium text-white bg-white/20 backdrop-blur-sm border border-white/30 hover:bg-white/30 transform hover:scale-105 transition-all duration-300">
                            إنشاء حساب
                        </a>
                    @endif
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" 
                                   name="remember" 
                                   class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 ml-2">
                            <span class="text-gray-700 font-medium">تذكرني</span>
                        </label>
